...
function renderPosts(){ feed.innerHTML = ''; const arr = loadPosts(); if(arr.length===0){ const samples = [
  ...
  // ensure sample/post s1 references CampusNet.png when available
  try{
    ...
    const existing = arr.find(p=>p.id === 's1');
    if(existing && !existing.media){ existing.media = 'CampusNet.png'; saveAllPosts(arr); }
  }catch(e){}

  loadPosts().forEach(post=>{
    // normalize legacy posts
    if(!Array.isArray(post.likedUsers)) post.likedUsers = [];
    const node = createPostElement(post);
    // attach likes/comments to elements
    ...
    if(currentUser && post.likedUsers.indexOf(currentUser) > -1){
      likeBtn.classList.add('liked');
    } else {
      likeBtn.classList.remove('liked');
    }
    // media rendered inside createPostElement
    // comment count
    const commentsEl = node.querySelector('.comments'); commentsEl.textContent = (post.comments||[]).length;
    // wire actions to persist
    ...
    likeButton.addEventListener('click', ()=>{
      ...
      if(idx>-1){
        ...
        if(uidx > -1){
          // unlike
          posts[idx].likedUsers.splice(uidx,1);
        } else {
          posts[idx].likedUsers = posts[idx].likedUsers || [];
          posts[idx].likedUsers.push(currentUser);
        }
        // keep legacy likes number in sync (optional)
    ...
    // add delete button (only post author can delete)
